...
def view_cart(request, pk):
    ...
    item_price_sum = sum(item.quantity * item.products.price for item in cart_list)
            ...
            order_items.append(
                {
                    ...
                    "amount": item.quantity * item.products.price,
                }
            )

        # pay 함수로 리다이렉트
        return redirect("shop:pay_cart", user.pk)
